...
PT_STOP = {
    ...
    "isso","esta","este","isto","foi","foram","há","isso","já","la","lhe","lhes","mais","mas","me","mesmo",
...
def build_sbert(df: pd.DataFrame, model_name: str, batch: int = 32) -> None:
    if not USE_SBERT:
        logging.info("[emb] SBERT desativado (USE_SBERT=False).")
        return
    try:
...
def run() -> None:
    ...
    with closing(sqlite3.connect(str(RAW_DB))) as raw, \
        ...
        for _, rec in tqdm(raw_df.iterrows(), total=len(raw_df), desc="limpeza", unit="pdf"):
            ...
            if not str(lang).lower().startswith("pt"):
                dropped_lang += 1
                continue
